...
resource "aws_ec2_subnet_cidr_reservation" "this" {
  for_each = { for k, v in var.subnets : k => v if var.create && v.cidr_reservations != null }
...
resource "aws_ram_resource_association" "this" {
  for_each = { for k, v in var.subnets : k => v if var.create && var.resource_share_arn != null }
...
resource "aws_route_table" "this" {
  for_each = { for k, v in var.subnets : k => v if local.create_route_table }
...
resource "aws_route_table_association" "subnet" {
  ...
  subnet_id      = aws_subnet.this[each.key].id
  route_table_id = var.create_route_table ? aws_route_table.this[each.key].id : each.value.route_table_id
}
